...
menuTree.prototype={
	...
	addChild:function(str,id,children,check,link){//生成单项item-nav
		...
		for(var i=0,l=children.length;i<l;i++){
			...
			str.push('<div class="item-nav'+(hasChild?' parent':'')+'" id="'+id+'_'+i+'"><b></b>'+(check?'<span class="'+(children[i].checked?'onClick':'')+'"></span>':'')+'<a target="_self"' + (link?' href="'+children[i].url+'"':'') + attr + title + '>'+ children[i].name +'</a></div>');
