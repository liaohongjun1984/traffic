								...
								<tr>
									...
									<td style="width: 60%;">
										<div class="box-item"
											...
											<input type="hidden" name="parAreaCode" id="parAreaCode"
												value="" /> <input type="hidden" name="areaCode"
												id="areaCode" value="" /> <input type="hidden" name="flag"
												id="flag" value="0" />
											<table id="f_tb" class="form" style="margin-top: 0;">
												<tbody>
													<tr>
														<th><em class="em">*</em>上级区域：</th>
														<td><input type="text" class="validate[required]"
															id="parentName" name="parentName" placeholder="请输入用户账号"
															readonly="readonly" /></td>
													</tr>
													<tr>
														<th><em class="em">*</em>区域名称：</th>
														<td><input type="text" class="validate[required]"
															id="areaName" name="areaName" placeholder="请设置登录密码" /></td>
													</tr>
													<tr>
														<th><em class="em">*</em>邮政编码：</th>
														<td><input type="text" class="validate[required]"
															id="areaNum" name="areaNum" placeholder="请输入用户手机号码" /></td>
													</tr>
													<tr>
													...
													<tr>
														<td colspan="2">
															<div class="nav" style="padding-left: 20px;">
																<input type="button" class="button" id="update_area"
																	value="保存" />&nbsp;&nbsp;&nbsp;&nbsp;<input
																	type="button" class="button" id="savenew_area"
																	value="新增子机构" />&nbsp;&nbsp;&nbsp; <input
																	type="button" class="button" id="delete_area"
																	value="删除" />
															</div>
														</td>
													</tr>
...
<script>
	...
	$(function() {
		...
		$('#update_area').click(addAndUpdate);

		function addAndUpdate() {
			var flag = $('#flag').val();
			if (flag == 0) {
				callback_updateAreaTree();
			} else if (flag == 1) {
				alert("新增区域子节点");
				callback_addAreaItem();
			}

		}
		//保存修改的区域信息
		function callback_updateAreaTree() {

			var req = $('#f_1').serialize();
			var url = '/admin/user/modifiyAreaInfo.do';
			$.getJSON(url, req, function(json) {
				...
				$.sysop.popup.autoTip(
						'修改区域信息[' + $('#userName').val() + ']成功.', function() {
							window.location.href = "/admin/user_arealist.jspx";
						});
			});
		}

		//新增节点
		function callback_addAreaItem() {
			var req = $('#f_1').serialize();
			var url = '/admin/user/addAreaInfo.do';
			$
					.getJSON(
							url,
							req,
		...
		//新增子节点
		$("#savenew_area").click(function() {
			var param = {};
			param["parAreaCode"] = $("#areaCode").val();
			param["parentName"] = $("#areaName").val();
			alert($("#parAreaCode").val());
			$.post('/admin/user/queryParAreaInfo.do', param, function(data) {
				if (data.success) {
					$("#areaCode").val(data.object.areaCode);
					$("#parAreaCode").val(data.object.parAreaCode);
					$("#parentName").val(data.object.parentName);
					$('#flag').val("1");
					$("#areaName").val("");
					$("#areaNum").val("");
					$("#serialNum").val("");
		...
		$('#delete_area').click(callback_deleteAreaTree);
		//删除的区域信息
		function callback_deleteAreaTree() {

			var param = {};
			param["areaCode"] = $("#areaCode").val();
			alert($("#areaCode").val());
			$.post('/admin/user/removeAreaInfo.do', param, function(data) {
				if (data.success) {
		...
		//区域节点树
		function loadAreaInfoTree(setting) { //setting : {unfold:true,checkbox:false,editable:true,link:false}
			$
					.getJSON(
							'/admin/user/listAllAreaInfos.do',
							function(data) {
								if (data.success) {
																							...
																							function(
																								...
																								if (data.success) {
																									$(
																											"#areaCode")
																											.val(
																													data.object.areaCode);
																									$(
																											"#parAreaCode")
																											.val(
																													data.object.parAreaCode);
																									$(
																											"#areaName")
																											.val(
																													data.object.areaName);
																									$(
																											"#areaNum")
																											.val(
																													data.object.areaNum);
																									$(
																											"#parentName")
																											.val(
																													data.object.parentName);
																									$(
																											"#serialNum")
																											.val(
																													data.object.serialNum);
																									$(
																											'#flag')
																											.val(
																													"0");
